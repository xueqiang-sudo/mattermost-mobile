...
const enhance = withObservables(['channel', 'shouldHighlightActive', 'shouldHighlightState', 'isOnHome'], ({
    ...
    const isMuted = shouldHighlightState ?myChannel.pipe(
        switchMap((mc) => {
            if (!mc) {
                return of$(false);
            }
    ...
    const teamDisplayName = teamId ?observeTeam(database, teamId).pipe(
    ...
    const membersCount =
        ...
        channel.type === General.PRIVATE_CHANNEL ?queryChannelMembers(database, channel.id).observeCount(false) :of$(0);

    const isUnread = shouldHighlightState ?myChannel.pipe(
        switchMap((mc) => of$(mc?.isUnread)),
        distinctUntilChanged(),
    ) : of$(false);

    const mentionsCount = shouldHighlightState ?myChannel.pipe(
        switchMap((mc) => of$(mc?.mentionsCount ?? 0)),
        distinctUntilChanged(),
    ) : of$(0);

    const messageCount = shouldHighlightState ?myChannel.pipe(
    ...
    const lastPost = isOnHome ?lastPostSource.pipe(
        throttleTime(HOME_CONVERSATION_LAST_POST_THROTTLE_MS, asyncScheduler, {leading: true, trailing: true}),
        shareReplay({bufferSize: 1, refCount: true}),
    ) :lastPostSource;
    const lastPostAt = (isOnHome && shouldHighlightState) ?combineLatest([lastPost, myChannel]).pipe(
        map(([post, mc]) => Math.max(post?.createAt ?? 0, mc?.lastPostAt ?? 0)),
        distinctUntilChanged(),
    ) : of$(0);

    const lastPostWithFiles = isOnHome ?lastPost.pipe(
        switchMap((post) => {
            if (!post) {
    ...
    const lastPostPreview = isOnHome ?combineLatest([lastPostWithFiles, channelInfoObservable]).pipe(
    ...
    const lastPostType = isOnHome ?lastPost.pipe(

---
name: 部门详情加载与导航
overview: 使用 getSubDepartments + getDepartmentWithEmployees 组合加载部门子部门与成员，参考设计图实现可点击层级导航条和样式调整。
todos: []
isProject: false
---
...
- **新增** `fetchDepartmentDetail(departmentId, companyId)`，内部：
  ...
  - 调用 `ContactService.getDepartmentWithEmployees(departmentId)` 获取本部门成员（替代现有的 `getEmployeesOfDepartment`）
  - 使用 `Promise.all` 并行请求，返回 `{ subDepartments, employees }`
- 部门直接下属员工可调用 `ContactService.getDepartmentWithEmployees`（`/api/v1/departments/:id/employees`）。

> **说明**：`getSubDepartments` 目前用 `getCompanyWithDepartments('tmpteam1001')` 并按 `parent_id` 过滤。若无独立子部门接口，可在 `fetchDepartmentDetail` 中改为 `fetchDepartmentsOfCompany(companyId)` 后按 `parent_id === departmentId` 过滤，而不改 ContactService。

### 1.2 部门详情页使用新接口

在 [app/screens/home/contacts/department_detail/index.tsx](app/screens/home/contacts/department_detail/index.tsx)：

- 将 `useEffect` 中的并行请求替换为 `fetchDepartmentDetail(departmentId, companyId)`
...
- [app/actions/remote/contact.ts](app/actions/remote/contact.ts)：`fetchDepartmentDetail` 改造
...
flowchart TD
    ...
    subgraph DataLoad [数据加载]
        C -->|fetchDepartmentDetail| J[getSubDepartments + getDepartmentWithEmployees]
